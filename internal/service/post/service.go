...
func (s *ProviderService) getProviderConfig(ctx context.Context, userID string, providerName string) (*provider.ProviderConfig, error) {
	db, err := s.dbManager.GetDB(userID)
	if err != nil {
...
func (s *ProviderService) updateProviderConfig(ctx context.Context, userID string, providerName string, config *provider.ProviderConfig) error {
	db, err := s.dbManager.GetDB(userID)
	if err != nil {
		return fmt.Errorf("failed to get database: %w", err)
	}
